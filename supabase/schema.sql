...
create table if not exists public.vehicles (
  ...
  -- Retiré de la flotte (vendu, hors service définitif) : conserve tout
  -- l'historique (réparations, pannes, pleins, documents) contrairement à
  -- une suppression définitive, réservée aux véhicules sans historique réel.
...
create index if not exists vehicles_retired_idx on public.vehicles (retired);
...
-- 7bis. Réparations véhicules ----------------------------------------------------
-- Suivi du coût réel des réparations, avec facture optionnelle et lien
-- facultatif vers le signalement de panne d'origine. Boss uniquement partout
-- (contrairement à vehicle_issues qui autorise aussi les chauffeurs) : même
-- schéma RLS que vehicle_documents ci-dessus.
...
create index if not exists vehicle_repairs_vehicle_idx
  on public.vehicle_repairs (vehicle_id, repaired_at desc);

create policy "vehicle_repairs_boss_select"
  on public.vehicle_repairs for select using (public.is_boss());

create policy "vehicle_repairs_boss_insert"
  on public.vehicle_repairs for insert with check (public.is_boss());

create policy "vehicle_repairs_boss_update"
  on public.vehicle_repairs for update using (public.is_boss()) with check (public.is_boss());

create policy "vehicle_repairs_boss_delete"
  on public.vehicle_repairs for delete using (public.is_boss());
...
-- 11. Congés : demande chauffeur → validation patron --------------------------
-- Une fois approuvée, alimente automatiquement schedule (type 'conge',
-- source 'prevu') — même mécanisme que la création manuelle côté patron.
...
-- lire les lignes schedule d'un autre chauffeur — cette fonction security
-- definer contourne volontairement cette restriction mais ne renvoie QUE des
-- dates (jamais driver_id, note, ni aucune autre colonne).
...
-- 12. Notifications push (Module B) --------------------------------------------
-- RLS "propriétaire uniquement" au sens littéral (auth.uid() = user_id), pas
-- restreint à is_boss() : le module ne cible que le patron aujourd'hui côté
-- UI, mais la table elle-même ne fait aucune hypothèse sur le rôle pour
-- rester ouverte à une extension future (ex. chauffeur) sans migration
-- supplémentaire.
